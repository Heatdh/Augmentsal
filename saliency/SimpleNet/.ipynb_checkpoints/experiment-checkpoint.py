...
if __name__ == '__main__':
    # add a parser to either run all experiments or a specific one
    import argparse
    parser = argparse.ArgumentParser(description='Run experiments')
    parser.add_argument('--useaug', type=int, default=0, help='to tell if we want to use augmented data or not')
    args = parser.parse_args()
    use_aug = args.useaug
    if use_aug == 1:
        print("Using augmented data")
        json_dir = Path(__file__).parent.parent/"cfg_augment"
        json_files = [pos_json for pos_json in os.listdir(json_dir) if pos_json.endswith('.json') and not pos_json.startswith('default')]
        # read the jsson file index 0
        with open(str(json_dir)+"/"+json_files[0], 'r') as j:
            dict_config = json.loads(j.read())

        augmentation_methods = dict_config["augmentation_methods"]
        for i in augmentation_methods:
            print("Augmentation method ", i)
        # make a new key in the dict containing methods name
